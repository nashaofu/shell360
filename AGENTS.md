...
# Set ANDROID_HOME; NDK_HOME is optional and adb does not need to be in PATH.
...
- After making changes, determine which parts of the codebase were modified:
  ...
  - **Native Android code**: run `pnpm run android:dev` or `pnpm run android:build`. The cross-platform Node.js runner selects `gradlew`/`gradlew.bat`, resolves the SDK from `ANDROID_HOME`, and uses `NDK_HOME` or the latest version under `$ANDROID_HOME/ndk`.
