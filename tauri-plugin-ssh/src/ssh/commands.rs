            ...
            if let Some(msg) = unbounded_receiver.recv().await {
                match msg {
                    ...
                    UnboundedChannelMessage::ChannelEof(channel_id) => {
                        ...
                        if is_shell_channel {
                            message_channel
                                .send(MessageChannelData::Disconnect(DisconnectReason::Server))?;
                        }
                    }
                    ...
                    UnboundedChannelMessage::Request(tx, remote_address, remote_port) => {
                        let remote_port_forwardings =
                            ssh_manager.remote_port_forwardings.lock().await;
...
pub async fn ssh_open_local_port_forwarding<R: Runtime>(
    ...
    let mut receiver = {
        ...
        let mut local_port_forwarding_senders =
            ssh_manager.local_port_forwarding_senders.lock().await;
