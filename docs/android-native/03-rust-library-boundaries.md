...
将现有 Tauri 插件中的业务实现提取为独立 Rust 库，同时保持桌面 Tauri 功能可用，
...
tauri-plugin-data --> shell360-data
tauri-plugin-pty  --> shell360-pty

shell360-ffi --> shell360-ssh
             --> shell360-data
             --> shell360-keygen
             --> shell360-pty（可选 desktop feature）
```
...
- Android KeyStore 等具体平台 API。

数据库目录、配置目录和认证状态事件通过明确接口提供。

### shell360-pty

包含本地 PTY 进程、输入输出、resize 和退出管理，只服务支持 PTY 的桌面平台。
...
1. 在新业务库中建立无 Tauri 的公开 API。
2. 将现有实现迁移到业务库。
3. 让 `tauri-plugin-*` 调用业务库。
4. 桌面回归通过后再从 `shell360-ffi` 调用同一业务库。

不采用一次性删除 Tauri 插件的方式，避免同时破坏桌面和移动端。

## 共享类型原则

- 领域类型留在各自业务库。
- 不创建大而全的 `shell360-core` 或公共 `types` crate。
- 只有出现两个以上 crate 必须共享且语义稳定的基础类型时，才考虑小型公共 crate。
- FFI 和 Tauri adapter 各自负责边界类型转换。

## 异步运行时

- 业务库只暴露 async API，不自行创建全局 Tokio runtime。
- Tauri 使用现有异步 runtime 调用。
- `shell360-ffi` 为 Android/iOS 持有一个共享 runtime。
- 禁止 SSH 和 data 各自创建独立全局 runtime。
...
- Android 构建不编译 `tauri-plugin-*`。
- `shell360-ssh` 与 `shell360-data` 不存在相互依赖。
- 桌面功能在提取后保持现有行为。
